...
"""
Utility for logging to a specified file using the 'logging' module
from Python standard library. In the 'logging module', a 'level' is 
associated to each message. The possible levels are: 
...
The log2file module currently implements the following behavior:
	...
	- if verbose option is set to True, all messages are also printed to sys.stdout
...
def get_log(log_file, verbose=False):
	...
	log_filter = LevelFilter([logging.DEBUG, logging.WARNING, logging.ERROR,
							  logging.CRITICAL])
	...
	log_handler.setLevel(logging.DEBUG)
	if verbose:
		log_filter = LevelFilter([logging.DEBUG, logging.INFO, logging.WARNING,
