...
def train_test_split(corpus_path, split_speakers=False,
    ...
    """Split utterances in a abkhazia corpus into train and test sets

    The results are put in a subfolder of the 'data' directory of the
    corpus named 'split'.

    Only one of train_amount, train_proportion and train_speakers can
    be set at the same time (default is train_proportion=0.5)

    train_speakers cannot be set if split_speakers is True

    If the split_speakers argument is False, the data for each speaker
    is attributed to one of the two sets as a whole.

    If train_speakers is set, it determines which speaker goes in
    which set. Otherwise train_amount (or a proportion
    train_proportion) of the speakers are randomly selected to be in
    the training set. Note that this might not be appropriate when the
    amount of utterances available per speaker is too unbalanced.

    If the split_speakers argument is True, both sets get speech from
    all the speakers with a number of utterances by speaker in each
    set matching the number of utterances by speaker in the whole
    corpus.  The total number of utterances in the training set is
    equal to either train_amount or
    round(train_proportion*total_nb_utt).

    """
    # parse arguments
    assert os.path.isdir(corpus_path), "{0} is not a directory".format(corpus_path)

    data_dir = os.path.join(corpus_path, 'data')
    assert os.path.isdir(data_dir), "{0} is not a directory".format(data_dir)

    split_dir = os.path.join(data_dir, 'split')
    if not os.path.isdir(split_dir):
        os.mkdir(split_dir)

    train_dir = os.path.join(split_dir, train_name)
    if os.path.isdir(train_dir):
        raise ValueError("There is already a split called: {0}"
                         .format(train_dir))
    else:
        os.mkdir(train_dir)

    test_dir = os.path.join(split_dir, test_name)
    ...
    utt2spk_file = os.path.join(data_dir, 'utt2spk.txt')
    assert os.path.exists(utt2spk_file), "{0} file does not exist".format(utt2spk_file)
    ...
    if (sp and am) or (sp and pr) or (am and pr):
        raise ValueError(
            'Specify only one of train_amount, '
            'train_proportion and train_speakers')
    ...
    # read input
    utt_ids, utt_speakers = io.read_utt2spk(
        os.path.join(corpus_path, 'data', 'utt2spk.txt'))
    ...
    if split_speakers:
        ...
        for speaker in speakers:
            spk_utts = [utt_id for utt_id, utt_speaker in utts
        ...
        if am or pr:
            ...
            missing_speakers = [spk for spk in train_speakers if spk not in speakers]
    ...
    try:
        ...
        for f in ['utt2spk.txt', 'text.txt', 'segments.txt']:
            train_out = os.path.join(train_dir, f)
            test_out = os.path.join(test_dir, f)

            io.copy_first_col_matches(
                os.path.join(data_dir, f), train_out, train_utt_ids)

            io.copy_first_col_matches(
                os.path.join(data_dir, f), test_out, test_utt_ids)
