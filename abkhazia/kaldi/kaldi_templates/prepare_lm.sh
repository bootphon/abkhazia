...
###
#
##  Not yet tested on n-grams with n!=2 or on non-word-position-dependent acoustic models
#
###
...
# First need to do a prepare_lang in the desired folder to get to use the right
# "phone" or "word" lexicon irrespective of what was used as a lexicon in training.
# If word_position_dependent is true and the lm is at the phone leve 
# use prepare_lang_wpdpl.sh in the local folder, otherwise we fall back to the
# original utils/prepare_lang.sh (some slight customizations of the script are
# necessary to decode with a phone loop language model when word
# position dependent phone variants have been trained).
if [ "$word_position_dependent" = true ] ; then
	# empty phone file is used to signal if the LM is not at the word but at the phone
	# level
