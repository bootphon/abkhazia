...
"""This script contains functions to train an ABnet for a list of
pairs of same words and to calculate the embeddings using a trained
ABnet.
...
def train(same_pair_file, output_path,
    ...
    It takes as input a list of pairs of same words. This file
    contains several lines in the format:

        word_id wav1 start1 stop1 wav2 start2 stop2 spk-id1 spk-id2

    - word_id is a word identifier used for sampling different
      words: 2 different words cannot have the same id. If you do
      not have labels, you can use a counter.
    ...
    same_pair_file: string
        path to the input file. This file contains a list of pairs of
        same words.
    ...
    same_word_ratio: float
        ratio "number of same word pairs" / "total number of pairs"
        for each training batch. Default to 0.5

    same_speaker_ratio: float
        ratio "number of same speaker pairs" / "total number of pairs"
        for each training batch. Default to the initial ratio

    train_ratio: float
        proportion of the data used for training (the
        rest are used for validation, to avoid overfitting)
    ...
    batch_size: int
        number of pairs (of frames ?) per batch. Tune this to the
        memory of your gpu so that it is fully used
...
def decode(model_path, wavefile_list, output_file,
    ...
    prepare.h5features_fbanks(
        wavefiles, stacked_fb_file,
    ...
    def do_transform(fname):
        times, feats = h5features.read(
            stacked_fb_file, from_internal_file=fname, index=index)
        times = times[fname]
        feats = feats[fname]
    ...
    prepare.h5features_fbanks(
        index['files'], output_file, featfunc=do_transform)
    if verbose:
...
- pairs_mean_std.npz: numpy archive file of the mean and standard
...
def train_from_features(same_pair_file, output_path, features_file,
                        ...
                        batch_size=100,
                        init_lr=0.01):
    """Function to train an ABnet on a list of words with personalised
    features
    ...
    - word_id is a word identifier used for sampling different words:
      2 different words cannot have the same id. If you do not have
      labels, you can use a counter.
    - wav1 and wav2 are wavefiles specified with their full path
    - start1, start2, stop1, stop2 are times in seconds
    - spk-id1 and spk-id2 are strings identifying uniquely the speakers
    ...
    train_ratio: float
        proportion of the data used for training (the rest are used
        for validation, to avoid overfitting)

    features_file: string
        path to the h5features file to use nframes: int number of
        frames to stack in the input.

    max_epochs: int
        maximum number of batch iterations batch_size: int number of
        pairs (of frames ?) per batch. Tune this to the memory of your
        gpu so that it is fully used
...
def decode_from_features(model_path, wavefile_list, output_file,
                         features_file, verbose=0):
    """Function to calculate the embeddings for a list of wavefiles using
    a trained ABnet in the h5features format
