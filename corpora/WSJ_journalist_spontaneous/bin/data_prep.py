...
"""
Data preparation WSJ for journalist spontaneous
"""
...
derived_path = "/fhgfs/bootphon/data/derived_data/new_WSJ_abkhazia/WSJ_journalist_spontaneous/"
dict_path = "/fhgfs/bootphon/data/raw_data/CMU_dict/"
abkhazia_path_data = "/fhgfs/bootphon/scratch/xcao/github_abkhazia/abkhazia/corpora/WSJ_journalist_spontaneous/data/"
abkhazia_path = "/fhgfs/bootphon/scratch/xcao/github_abkhazia/abkhazia/corpora/WSJ_journalist_spontaneous/"
...
#Otherwise, start from step 4 to just link the wavs rep. (wavs already available in /fhgfs/bootphon/data/derived_data/new_WSJ_abkhazia/WSJ_journalist_spontaneous/)
...
def copy_wv1_trs(o, o2):
    ...
    #walk recursively and find the directories for j_spontaneous: si_tr_jd and "s"
    #needs to change regex to find the correct files
    for dirpath, dirs, files in os.walk(raw_path):
        for d in dirs:
            d_match = re.match("si_tr_jd", d)
            if d_match:
    ...
    for d in array_good_dir:
        for dirpath2, dirs2, files2 in os.walk(d):
            for f in files2:
                m_file = re.match("^([0-9a-z][0-9a-z][0-9a-z])s(.*)\.wv1", f)
                if m_file:
                ...
                m_trs = re.match("^([0-9a-z][0-9a-z][0-9a-z])s(.*)\.dot", f)
                if m_trs:
                    filename = os.path.join(dirpath2,f)
...
def segments_text_speakers():
    ...
    for filename in files:
        ...
        m_speaker = re.match("^([0-9a-z][0-9a-z][0-9a-z])s(.*)\.wav", filename)
        if m_speaker:
