...
- [X] revoir l'architecture de abkhazia acoustic pour avoir une classe par
...
** acoustic
   ...
   - [ ] test the acoustic model (this is the decode command)
     providing WER for every model, eventual automatic parameter search ?
   - [X] expose in Kaldi parameters (e.g. in steps/train_mono.sh) to
     the abkhazia interface
** decode
   options:
